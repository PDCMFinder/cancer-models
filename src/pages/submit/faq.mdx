...
The six section of CancerModels.Org metadata template covers the clinical and model information using these six sheets: patient, patient sample, sharing, pdx model, model validation, cell model.
...
Recommended: Please use a unique RNA sequencing ID (sample ID) for each replicate, and provide a separate file for each replicate and named using the same ID.
